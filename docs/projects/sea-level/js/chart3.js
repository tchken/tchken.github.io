...
var update_sealevel3 = function(data, cb){
    ...
    // client-side replacement for the Flask /predict endpoint
    cb({ sea_level: window.predictSeaLevel(data.years, data.gt) });
}
...
document.getElementById("chart3years")
  .addEventListener("input", function(e){
    ...
    if (MASKS_LOADED === true){
        ...
        update_sealevel3({"gt":last_co2,"years":e.target.value}, cb);
    }
});
...
d3.json("data/tsung-chin.json").then(function(data){
  ...
  // setup margin
  var margin = {top:110, right:140, bottom:110, left:140};
